...
| **專業建築圖例與設施 90 度順時針旋轉功能 (90° Architectural Symbol & Room Rotation)** | 畫板 Modal 屬性面板 & SVG 渲染器 | 依據指示為所有專業建築圖例 (床位 🛏️、灶位 🍳、浴缸 🛁、馬桶 🚽、水槽 🚰、文昌桌 💼、氣口門 🚪、採光窗 🪟) 與房間分區新增 90 度旋轉機制：<br/>1. **屬性面板 `🔄 轉90°` 按鈕**：點選任何物件時，可在左側極簡卡片一鍵點擊順時針旋轉 90°。<br/>2. **鍵盤 `R` 鍵快速旋轉**：選取物件時按鍵盤 `R` 鍵即可快速旋轉！<br/>3. **向量圖形與邊框包絡自動對齊**：自動切換 `width` 與 `length` 邊框尺寸，並透過 SVG `transform="rotate(deg, cx, cy)"` 即時轉動內部 2D 向量結構 (如水箱、枕頭、爐嘴、門弧)，達到完美擬真放樣效果！ | ✅ 100% 正確 |
